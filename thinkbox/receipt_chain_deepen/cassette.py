"""Cassette replay for receipt-chain flows (PR #182 F09)."""
...
def cassette_path(name: str, repo_root: Path | None = None) -> Path:
    ...
    if safe.endswith(".json"):
        safe = safe[:-5]
    return root / "data/receipt_chain/cassettes" / f"{safe}.json"


def load_cassette(name: str, repo_root: Path | None = None) -> ChainCassette:
    path = cassette_path(name, repo_root)
    doc = json.loads(path.read_text(encoding="utf-8"))
    steps = tuple(dict(s) for s in (doc.get("steps") or []))
    return ChainCassette(name=name, steps=steps)
