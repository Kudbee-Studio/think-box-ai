...
| Autonomous decision loop: Durable action receipts | **#251** (draft) | `thinkbox/autonomous_loop_action_store.py` — SQLite append-only `loop_action_receipts` with SHA-256 hash chain (`GENESIS` head), `LoopActionStore.append/verify/latest/by_loop/count`; `DashboardState.set_loop_action_store()` so `record_loop_action()` persists and survives restart; `GET /actions/integrity` endpoint; tests `tests/unit/test_autonomous_loop_action_store.py` (tamper detection + reopen survival) |
