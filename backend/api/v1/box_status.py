...
box_status_router = APIRouter(prefix="/think/box-status")

_dashboard_state = get_dashboard_state()


@box_status_router.get("/status")
async def box_status() -> dict[str, Any]:
    ...
    await _dashboard_state.emit(
        ...
        {"component": "box_status", "substrate": substrate, "model": "mercury-2"},
        "box_status_api",
        evidence_label="verified" if substrate == "upstash-box" else "simulated",
    )
