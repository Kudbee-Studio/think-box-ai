"""Agent loop for kudbEE.

Orchestrates: goal → plan → act → observe → remember.
...
class AgentLoop:
    """The kudbEE brain — executes goals with streaming, tools, and memory."""
    ...
    async def run(self, goal: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Phase 3: Completion
            await event_bus.broadcast_thought("Goal completed successfully", status="success")
    ...
    async def _plan(self, goal: str) -> None:
        ...
        tools_desc = ", ".join([t.name for t in plugin_registry.list_tools()])
...
Plan your approach in 1-2 sentences. Be concise."""
    ...
    async def _execute_tool(self, tool_call: dict[str, Any]) -> None:
        ...
        await event_bus.broadcast_tool_result(tool_name, payload)
