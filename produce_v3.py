#!/usr/bin/env python3
"""KUDBEE Autonomous Production Loop v2 — Fixed

Fixes from Run #1:
- Ollama retry with backoff
- Simpler FFmpeg scene generation
- Pre-generated music fallback
...
def generate_scene(scenes_dir, scene_num, start, end, text):
    ...
    if text:
        # Escape for FFmpeg drawtext: newlines become \n, escape special chars
...
def main():
    print("=" * 60)
    print("  KUDBEE AUTONOMOUS PRODUCTION v2")
...
Continue for 12 scenes total. Each scene has timestamps and text.""", max_tokens=1500)
    ...
    subprocess.run(["espeak", "-w", wav_file, "-s", "140", narration], 
    ...
    # === MUSIC (use pre-generated or fallback) ===
    print("\n[MUSIC] Using music...")
    music_mp3 = f"{OUTPUT_DIR}/score.mp3"
    
    # Try pre-generated music first
    prefabs = [
    ...
    manifest["boxes"]["music"] = {"status": "ready", "file": music_mp3}
    print(f"  Music: {'found' if os.path.exists(music_mp3) else 'silent fallback'}")
    
    # === EDITOR (assemble scenes) ===
    ...
    print(f"  Scenes generated: {generated}/{len(scene_data)}")
