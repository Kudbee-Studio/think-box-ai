#!/usr/bin/env python3
"""Print KILO live-smoke-evidence contract summary (PR #152, hermetic operator gate)."""
...
from thinkbox.kilo_post_season_harden import hermetic_post_season_harden_operator_check
from thinkbox.kilo_substrate_checklist import BOX_URL_ENV


def main() -> int:
    parser = argparse.ArgumentParser(description="KILO live-smoke-evidence hermetic verify")
    parser.add_argument(
        "--live",
        action="store_true",
    ...
    post = hermetic_post_season_harden_operator_check()
    if not post.ok:
        print(json.dumps({"error": "prior post-season-harden operator failed"}, indent=2))
        return 1

    summary = live_smoke_evidence_contract_summary()
                ...
                json.dumps(
                    {
                        ...
                        "probe_steps": [
                            "export THINKBOX_SWARM_LIVE_ACK=1",
                            "export UPSTASH_PUBLIC_BOX_URL=https://<box>.box.upstash.com",
                            "run founder bounded smoke; write data/thinkboxmd/artifacts/kilo_live_smoke_*.json",
                            "run audit_flip_candidate; write docs/audit/passes/*-live.json",
                        ],
                    },
                    indent=2,
                ),
