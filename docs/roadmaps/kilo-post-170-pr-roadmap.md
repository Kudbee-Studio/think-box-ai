# KILO post-#170 PR roadmap (#171–#180)
...
|------|------------------|---------|------|--------------|-------|
| **#171** | CI spine-trust slimming (dedupe workflow verify scripts) | CI runs spine **and** nearly every gate script twice; wall-clock + flake surface; aligns with (a) spine health | ~8–15 commits; `test.yml`, `kilo_post_season_harden` checklist/docs, `test_kilo_live_proof_readiness_pr171` | Removing gate modules; combined umbrellas; live smoke | Must keep `verify_kilo_beyond_kilo_lint.py` with `EXECUTE=1` in CI; post_season checklist drift |
| **#172** | Chronicle honesty sync (#170 merged + post-170 era) | AGENTS/CONTINUITY/runbook H30 vs “draft”; audit index; (c) docs honesty | ~5–10 commits; docs + hermetic spine-doc tests only | Production code except test fixtures for doc paths | Low; avoid affirmative LIVE claims |
| **#173** | Lint scope wave 1 (spine + hermetic subprocess helpers) | #170 explicitly deferred widening; (b) gradual lint | ~6–12 commits; `LINT_SCOPE_REL_PATHS` + ruff/mypy fixes on new files | Whole `thinkbox/` tree; `core/` runtime | Mypy time; keep `--follow-imports=skip` |
| **#174** | Lint scope wave 2 (live-proof readiness spine modules) | Next bounded slice: `kilo_live_proof_readiness`, `kilo_env_matrix`, `kilo_hermetic_gate_memo` | ~8–15 commits | Combined lanes; importing provider SDKs | Same as #173 |
| **#175** | Control-plane receipt-chain single deepen (412/precondition only) | Mature receipt/ETag stack (#155–#161); one HTTP edge theme | ~10–18 commits; `receipt_chain_query` / conditional GET tests | END_LINK UX; dashboard bind; api_ops combined | No live Mercury; hermetic e2e only if scoped |
| **#176** | Control-plane ops harden slice (rate-limit / error shape only) | Thin follow-on after #157/#161 without post-NNN combined pattern | ~8–14 commits | New dashboard pages; swarm/gov themes | Duplicating #169 ops deepen |
| **#177** | Think Job hermetic e2e deepen (F023+ control-plane poll/SSE) | CONTINUITY: F023 on main; live API path still uncovered | ~12–20 commits; `tests/e2e/` only + minimal route stubs | Live `POST /run` Mercury; GPU | Nested e2e — **not** in default spine; document `--e2e` |
| **#178** | KUDBEECLI Phase 2 merge prep (single theme: persist + trace list) | #128 merged; #129 draft ages; isolate persist/trace before REPL/live gate | ~15–25 commits | `swarm live` HTTP; agent register | Scope creep into Phase 3 |
| **#179** | Nightly / manual `spine --e2e` workflow | Keeps deep control-plane e2e without blocking every PR | ~4–8 commits; new workflow `workflow_dispatch` + schedule | Making `--e2e` default on PR CI | Runner cost if scheduled too often |
| **#180** | Founder optional: bounded live-smoke **runbook + operator dry-run** only | When Box URL/token/LIVE_ACK exist; documents #153 path | ~6–10 commits; runbook + hermetic operator tests | Executing live proof in CI; `live_verified: true` in repo | Credential leakage in docs; cost |
...
## Recommended default next PR: **#171**

See launch brief in repo root agent handoff (coordinator copies from this doc § Launch brief).

---

## Launch brief — #171 CI spine-trust slimming

**Branch:** `cursor/pr171-ci-spine-trust-slim-8f6c`  
...
3. Add `tests/unit/test_kilo_live_proof_readiness_pr171.py` asserting CI manifest contract (no combined umbrella).
4. Audit pass `docs/audit/passes/2026-09-24-pr171.json` with `live_verified: false`.
5. Touch `docs/runbooks/kilo-live-proof-readiness.md` § Hermetic prerequisites with **H31** CI-trust note (optional one paragraph).

**Out of scope:** Deleting `scripts/verify_kilo_*.py` (operators still run them locally). Changing spine block list. Lint scope widen (#173). Live proof. Any #172 docs-only chronicle (separate PR).
...
python3 -m unittest tests.unit.test_kilo_live_proof_readiness_pr171 -v
```
...
When **#171** merges, update `docs/CONTINUITY.md` CURRENT STATE: PR #170 merged; active objective = CI spine-trust; link this file.
