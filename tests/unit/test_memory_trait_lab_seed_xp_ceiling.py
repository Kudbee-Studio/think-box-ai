"""Hermetic tests for Trait Lab seed history XP ceiling (PR #218)."""
...
from thinkbox.memory_layers import (
    ...
    trait_lab_seed_history_by_xp_ceiling,
)
from thinkbox.trait_game.engine import act, load_rules, new_run, proof_scorecard


class TestMemoryTraitLabSeedXpCeiling(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_filter_keeps_runs_at_or_below_ceiling(self) -> None:
        first = self._record_finish(5, "pr218-finish")
        second = self._record_draw_finish(5, "pr218-draw")
        low = min(int(first["xp"]), int(second["xp"]))
        high = max(int(first["xp"]), int(second["xp"]))
        filtered = trait_lab_seed_history_by_xp_ceiling(self.store, 5, high)
        self.assertEqual(filtered["seed"], 5)
        self.assertEqual(filtered["ceiling"], high)
        self.assertGreaterEqual(filtered["count"], 1)
        self.assertTrue(all(int(row["xp"] or 0) <= high for row in filtered["runs"]))
        self.assertEqual(filtered["best"]["xp"], high)
        self.assertFalse(filtered["live_verified"])
        tight = trait_lab_seed_history_by_xp_ceiling(self.store, 5, low)
        self.assertTrue(all(int(row["xp"] or 0) <= low for row in tight["runs"]))
        self.assertEqual(tight["best"]["xp"], low)

    def test_filter_excludes_other_seeds(self) -> None:
        proof = self._record_finish(1, "pr218-a")
        self._record_finish(2, "pr218-b")
        filtered = trait_lab_seed_history_by_xp_ceiling(self.store, 1, int(proof["xp"]))
    ...
    def test_missing_and_invalid_inputs(self) -> None:
        with self.assertRaises(MemoryLayerError) as err:
            trait_lab_seed_history_by_xp_ceiling(self.store, 99, 0)
        self.assertEqual(err.exception.code, "missing_seed")
        proof = self._record_draw_finish(3, "pr218-c")
        self.assertGreater(int(proof["xp"]), 0)
        with self.assertRaises(MemoryLayerError) as err2:
            trait_lab_seed_history_by_xp_ceiling(self.store, 3, int(proof["xp"]) - 1)
        self.assertEqual(err2.exception.code, "missing_ceiling")
        with self.assertRaises(MemoryLayerError) as err3:
            trait_lab_seed_history_by_xp_ceiling(self.store, 3, "nope")  # type: ignore[arg-type]
        self.assertEqual(err3.exception.code, "invalid_ceiling")
        with self.assertRaises(MemoryLayerError) as err4:
            trait_lab_seed_history_by_xp_ceiling(self.store, 3, -1)
        self.assertEqual(err4.exception.code, "invalid_ceiling")
        with self.assertRaises(MemoryLayerError) as err5:
            trait_lab_seed_history_by_xp_ceiling(self.store, 3, int(proof["xp"]), limit=0)
