...
Provides full-text search (FTS5 + BM25) and hybrid search capabilities.
"""
...
class SearchEngine:
    ...
    def _get_context(self, msg_id: int, session_id: str, window: int) -> str:
        ...
        parts = []
        for row in rows:
            parts.append(f"[{row['role']}]: {row['content'][:200]}")
        return "\n".join(parts)
    ...
    def search_memory(
        ...
        if project_h:
            sql = """
                SELECT pm.id, pm.key, pm.value, pm.source,
                       bm25(memory_fts) as rank
                FROM memory_fts
                JOIN project_memory pm ON memory_fts.rowid = pm.id
                WHERE memory_fts MATCH ? AND pm.project_hash = ?
                ORDER BY rank
                LIMIT ?
            """
            rows = conn.execute(sql, [query, project_h, limit]).fetchall()
    ...
    def get_project_context(self, project: str) -> dict[str, Any]:
        """Get startup context for a project."""
        conn = self._conn()
        ...
        # Key memory
        memory = conn.execute("""
            SELECT key, value, source FROM project_memory
            WHERE project_hash = ?
            ORDER BY updated_at DESC
            LIMIT 20
        """, [project_h]).fetchall()
